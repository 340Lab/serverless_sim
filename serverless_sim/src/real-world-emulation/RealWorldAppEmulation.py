...
def getRandCV(rng):
    ...
    cv = utils.getRandFloatRefByCDF(cvCDFFile, rng)
    return cv

# 检查参数数量是否正确
if len(sys.argv) != 3:
    print("Usage: python your_script.py <IAT|CV> <argument_for_function>")
    sys.exit(1)


arg = sys.argv[1]  # 第一个参数
rng = float(sys.argv[2])  # 将第二个参数从字符串转换为浮点数


if arg == "IAT":
    result = getRandAvgIAT(rng)
elif arg == "CV":
    result = getRandCV(rng)
else:
    print("Invalid argument. Use 'IAT' or 'CV'.")
    sys.exit(1)
