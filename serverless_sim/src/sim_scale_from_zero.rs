...
use crate::{ sim_env::{ SimEnv }, sim_scale_executor::{ ScaleExecutor, ScaleOption } };
    ...
    fn scale_some(&mut self, env: &SimEnv) {
        ...
        for (_reqid, req) in env.requests.borrow().iter() {
            ...
            'outer: while let Some(f) = walker.next(&req_dag.dag_inner) {
                ...
                // scale up
                let mut scale_cnt = env.scale_executor.borrow_mut().scale_up(env, fnid, 1);
                if scale_cnt == 0 {
                    ...
                    scale_cnt = env.scale_executor.borrow_mut().scale_up(env, fnid, 1);
                    // assert_eq!(env.scale_executor.borrow_mut().scale_up(env, fnid, 1), 1);
                }
                if scale_cnt != 1 {
                    log::warn!(
                        "req {} fn {} scale up failed, resource is not enough",
                        req.req_id,
                        fnid
                    );
                } else {
                    log::info!("req {} fn {} scale up {} containers", req.req_id, fnid, scale_cnt);
                }
            }
        }
    }
