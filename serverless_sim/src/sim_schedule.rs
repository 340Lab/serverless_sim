...
impl SimEnv {
    fn try_put_fn(&self) {
        ...
        //  1.前驱fn已经执行完了
        //  2.fn instance 存在
        for (req_id, req) in self.requests.borrow_mut().iter_mut() {
    ...
    fn sim_computes(&self) {
        for n in self.nodes.borrow_mut().iter_mut() {
            if let Some((req_fns_2_run, starting_container_cnt, cpu_for_one_task)) =
