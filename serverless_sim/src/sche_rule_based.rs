...
use crate::{ schedule::Scheduler, sim_env::SimEnv, request::Request, fn_dag::FnId, node::NodeId };
...
impl SimEnv {
    ...
    fn schedule_fn_gofs(&self, req: &mut Request, fnid: FnId) {
        ...
        let most_space_nodes_transtime = most_space_nodes
            .iter()
            .map(|&n| { (n, self.algo_predict_fn_on_node_work_time(req, fnid, n, None)) })
            .collect::<Vec<_>>();
        ...
        for &n in nodes.iter() {
            let score = match rule {
                ...
                ScheduleRule::LeastLoad => {
                    self.node(n)
                        .fn_containers.iter()
                        .map(|(_, c)| -(c.req_fn_state.len() as f32))
                        .sum::<f32>()
                }
            };
