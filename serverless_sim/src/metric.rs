        ...
        for (_, req) in env.core.requests().iter() {
            ...
            while let Some(fngid) = walker.next(&env.dag(req.dag_i).dag_inner) {
                ...
                if !req.done_fns.contains_key(&fnid) {
                    fn_count.entry(fnid).and_modify(|v| *v += 1).or_insert(1);
                }
            }
        }

        for (fnid, count) in fn_count.iter() {
            // 如果fnid在fn_recent_req_cnt_window中，则将其窗口的队列的最后一个值加1.0
            self.fn_recent_req_cnt_window
               .entry(*fnid)
               .and_modify(|window| 
                        if window.queue().len() == window.cap() {
                            let mut queue = window.queue_mut();
                            queue.pop_front();
                            queue.push_back(*count as f32);
                        }
                        else{
                            let mut queue = window.queue_mut();
                            queue.push_back(*count as f32);
