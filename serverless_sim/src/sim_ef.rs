...
use crate::{
    ...
    node::NodeId,
    actions::{ RawAction, EFActionWrapper },
    sim_schedule::{ Scheduler, SchedulerImpl },
    sim_ef_lass::LassEFScaler,
    sim_ef_fnsche::FnScheScaler,
    network::{ Config },
    sim_ef_faas_flow::FaasFlowScheduler,
    sim_ef_hpa::HpaEFScaler,
};
...
#[enum_dispatch(EfScaler)]
pub enum EFScalerImpl {
    LassEFScaler(LassEFScaler),
    FnScheScaler(FnScheScaler),
    HpaEFScaler(HpaEFScaler),
}
...
#[allow(dead_code)]
pub const FRAME_BEGIN_IDX: usize = 0;
pub const SCALE_FOR_FNS_IDX: usize = 1;
pub const SCHEDULE_IDX: usize = 2;
#[allow(dead_code)]
pub const SIM_COMPUTE_IDX: usize = 3;
pub const SCALE_DOWN_IDX: usize = 4;
...
pub fn prepare_spec_scheduler(config: &Config) -> Option<SchedulerImpl> {
    if let Some(ef) = config.aief.as_ref() {
        if ef.sche_faas_flow() {
            return Some(SchedulerImpl::FaasFlowScheduler(FaasFlowScheduler::new()));
        }
    }
    None
}

pub fn prepare_spec_scaler(config: &Config) -> Option<EFScalerImpl> {
    if let Some(ef) = config.aief.as_ref() {
        if ef.scale_lass() {
            return Some(EFScalerImpl::LassEFScaler(LassEFScaler::new()));
        } else if ef.sche_fnsche() {
            return Some(EFScalerImpl::FnScheScaler(FnScheScaler::new()));
        } else if ef.scale_hpa() {
            return Some(EFScalerImpl::HpaEFScaler(HpaEFScaler::new()));
        }
    }
    None
}
...
impl SimEnv {
    ...
    pub fn step_ef(&mut self, raw_action: EFActionWrapper) -> (f32, String) {
        self.avoid_gc();

        let mut ef_state: RefMut<'_, EFState> = self.aief_state.as_ref().unwrap().borrow_mut();
        ...
        let config_ef = || { self.config.aief.as_ref().unwrap() };
        ...
        loop {
            if ef_state.stage.is_frame_begin() {
                ...
                if config_ef().sche_faas_flow() {
                    ef_state.trans_stage(self);
                    continue;
                }
                if config_ef().scale_ai() {
                    if
                        !sim_ef_ai::step_scale(
                            self,
                            &raw_action,
                            &mut action_done,
                            &mut action_score,
                ...
                log::info!("schedule");
                if self.config.aief.as_ref().unwrap().sche_ai() {
                    if action_done {
                    ...
                    let action = match raw_action {
                        EFActionWrapper::Float(raw_action) => (raw_action * 31.0) as u32,
                ...
                } else if self.config.aief.as_ref().unwrap().sche_rule() {
                    self.try_put_fn();
                    ef_state.trans_stage(self);
                } else if self.config.aief.as_ref().unwrap().sche_faas_flow() {
                    let mut spec = self.spec_scheduler.borrow_mut();
                    let scheduler = spec.as_mut().unwrap().as_faas_flow_scheduler_mut().unwrap();
                    scheduler.schedule_some(self);
                    ef_state.trans_stage(self);
                } else if self.config.aief.as_ref().unwrap().sche_fnsche() {
