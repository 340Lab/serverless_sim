...
# 调整子图的边距以确保图例不会覆盖图表内容
    ...
    plt.subplots_adjust(top=0.85,left=0.1,right=0.9,wspace=0.25, hspace=0.25)
    ...
    plt.legend(
        loc='lower center',
        bbox_to_anchor=(-1.42, 1.1),  # 调整位置到柱状图正上方
        fancybox=True,
        ncol=8,  # 增加列数，减小每列宽度
        fontsize='small'
    )
...
def pipeline():
    ...
    drawconf=yaml.safe_load(open(yamlfilepath, 'r', encoding='utf-8'))
