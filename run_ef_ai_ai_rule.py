...
flags.DEFINE_integer(
    'num_train_steps', int(200000), 'Number of training steps (environment steps or frames) to run per iteration.'
)
flags.DEFINE_integer(
...
def main(argv):
    ...
    train_env.reset()
    # print("train_env.observation_space.shape",train_env.observation_space.shape)
    obs = train_env.obs
    ...
    checkpoint = PyTorchCheckpoint(
        environment_name=FLAGS.environment_name, agent_name=AGENT_NAME, save_dir=FLAGS.checkpoint_dir
    )
    if next_iter>0:
        checkpoint.register_pair(('network', network))
        checkpoint.restore(FLAGS.checkpoint_dir+"/"+AGENT_NAME+"_Pong_"+str(next_iter-1)+".ckpt")
        network.eval()
...
for fname in files:
    pre=AGENT_NAME+"_Pong_"
    pre_i=fname.find(pre)
    end=".ckpt"
    end_i=fname.find(end)
    if pre_i>-1 and end_i>-1:
        # print(fname[pre_i+len(pre):end_i])
        sub=fname[pre_i+len(pre):end_i]
        # print(sub)
        if len(sub)>0 and '9'>=sub[0]>='0':
            iter_=int(sub)
            if iter_>next_iter:
                next_iter=iter_

next_iter+=1
print("next_iter",next_iter,AGENT_NAME)
if next_iter>4:
    os.remove("./checkpoints/"+AGENT_NAME+"_Pong_"+str(next_iter-5)+".ckpt")
