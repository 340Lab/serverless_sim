...
from mat.runner.shared.serverless_runner import ServerlessRunner as Runner
# from mat.envs.env_wrappers import ShareSubprocVecEnv, ShareDummyVecEnv

from proxy_env_mat import ProxyEnv2
...
class Envs:
    ...
    action_space=None
    env=None
    def __init__(self,env:ProxyEnv2):
        self.env=env
        obs=env.observation_space
        self.observation_space=[obs]
        self.share_observation_space=[obs]
        self.action_space=[env.action_space]
        print("observation_space",self.observation_space,
            "share_observation_space",self.share_observation_space,
            "action_space",self.action_space[0].n,self.action_space[0].shape)

    def step(self,actions):
        print("actions",actions,"len",len(actions),actions[0][0][0])
        assert len(actions)==1
        # obs, share_obs, rewards, dones, infos, available_actions
        obs, reward, done, info=self.env.step(int(actions[0][0][0]))
        return [obs],obs,[reward],[done],[info],[[i for i in range(self.action_space[0].n)]]

    def reset(self):
        obs=self.env.reset()
        
        return obs,[obs],[[i for i in range(self.action_space[0].n)]]
